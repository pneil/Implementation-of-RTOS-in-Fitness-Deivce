...
void OS_PeriodTrigger0_Init(int32_t *semaPt, uint32_t period){
	...
	Period0 = period;
	BSP_PeriodicTask_InitB(&RealTimeEvents,1000,0);
}
...
// enable is bit 3 in NVIC_EN0_R
	...
	__asm(" NOP");
	__asm(" NOP"); 
	GPIO_PORTD_AMSEL_R &= ~64;
	GPIO_PORTD_AFSEL_R &= ~64; 
	GPIO_PORTD_DEN_R |= 64;
	GPIO_PORTD_DIR_R &= ~64;
	GPIO_PORTD_PUR_R &= ~64;
	GPIO_PORTD_IS_R &= ~64;
...
//***IMPLEMENT THIS***
  ...
  // step 3 disarm interrupt to prevent bouncing to create multiple signals
    ...
    GPIO_PORTD_ICR_R = 64;  // acknowledge flag4
    OS_Signal(edgeSemaphore);          // signal SW1 occurred
		GPIO_PORTD_IM_R &= ~64; // disarm interrupt on PD6
